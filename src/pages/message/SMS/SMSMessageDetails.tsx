...
export const SMSMessageDetails = (): JSX.Element => {
    ...
    return (
        <div className="min-h-screen bg-gradient-to-br from-indigo-50 via-purple-50 to-pink-50 px-4 py-12">
            ...
            >
                <Card className="bg-white/90 backdrop-blur-lg rounded-3xl shadow-2xl overflow-hidden border border-white/60">
                    ...
                    <CardContent className="p-8 space-y-10">
                        ...
                        <Section title={t("common.basic_info")} icon={<Send className="w-5 h-5" />}>
                            ...
                            <Detail label={t("common.message_type")} value={<Badge variant="secondary">{msg.messageType.toUpperCase()}</Badge>} />
                            <Detail label={t("common.send_date")} value={formatDate(msg.send_at)} icon={<Calendar />} />
                            <Detail label={t("common.tracking_id")} value={msg.tracking_id || t("common.none")} icon={<Hash />} />
                            <Detail label={t("common.portal")} value={msg.portal || t("common.none")} icon={<Globe />} />
                        </Section>
                        ...
                        <Section title={t("common.client_response")} icon={<UserCheck className="w-5 h-5" />}>
                            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                                <Detail
                                    ...
                                    icon={msg.clicked_at ? <Link2 className="w-5 h-5 text-blue-600" /> : <Clock className="w-5 h-5 text-gray-500" />}
                                />
                                <Detail
                                    label={t("common.completed")}
                                    value={msg.completed ? t("common.yes") : t("common.no")}
                                    icon={msg.completed ? <CheckCircle2 className="w-5 h-5 text-green-600" /> : <Clock className="w-5 h-5 text-orange-600" />}
                                />
                                {msg.completed_at && (
                                    <Detail
                        ...
                        {msg.is_survey && msg.survey && (
                            <Section title={msg.survey.name} icon={<FileText className="w-5 h-5" />} subtitle={msg.survey.description}>
                                            ...
                                            >
                                                ...
                                                {question.type === "text" && (
                                                    <p className="text-gray-700 bg-white rounded-xl p-4 border">
                                                        {answer || <span className="italic text-gray-400">{t("common.no_answer")}</span>}
                                                    </p>
                                                )}
